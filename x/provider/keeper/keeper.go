...
// GetDsTc - an utility function for retriving data source and test case information from the oracle script
func (k *Keeper) GetDsTc(ctx sdk.Context, oScript string, valCount int) (dSourceObjs []types.AIDataSource, tCaseObjs []types.TestCase, providedFees sdk.Coins, err error) {
	...
	requiredFees, err := k.GetMinimumFees(ctx, aiDataSources, testCases, valCount, k.GetOracleScriptRewardPercentageParam(ctx))
	if err != nil {
		return nil, nil, nil, sdkerrors.Wrap(err, "Error getting minimum fees from oracle script")
	}

	// collect data source objects
	for _, dSource := range aiDataSources {
		dSourceObj, err := k.GetAIDataSource(ctx, dSource)
		if err != nil {
			return nil, nil, nil, err
		}
	...
	for _, tCase := range testCases {
		tCaseObj, err := k.GetTestCase(ctx, tCase)
		if err != nil {
			return nil, nil, nil, err
		}
		tCaseObjs = append(tCaseObjs, *tCaseObj)
	}
	return dSourceObjs, tCaseObjs, requiredFees, nil
}
